...
use Controller\ItensCompraController;
use Model\Product;
...
switch(Functions::formatUrl()){
    ...
    case '/search':
        ...
        $categorias = json_decode($_GET['categorias'] ?? '[]', true);
        $estilo = json_decode($_GET['estilo'] ?? '[]', true);
        $sizes = json_decode($_GET['sizes'] ?? '[]', true);
        $genders = json_decode($_GET['genders'] ?? '[]', true);
        $condictions = json_decode($_GET['condictions'] ?? '[]', true);
        
        echo json_encode(ProductController::selectSearch($search, $colors, $sizes, $genders, $condictions, $categorias, $estilo), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    ...
    case '/POST/delete/produtos':
        Functions::verifyPost('id_produto');
        Functions::verifyIdPost('id_produto');

        echo json_encode(ProductController::delete($_POST['id_produto']), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    break;
    ...
    case '/POST/usuario/profileUpdate':
        try{
            ...
            foreach($postArray as $post){
                $$post = Functions::verifyPost($post);
            }
            ...
            if($_POST['profile_photo'] === 'removing'){
                $pessoa->setProfilePhoto('http://localhost/tcc/API/UPLOADS/profilePhotos/imgPadrao.png');
            }else{
                if($_FILES['profile_photo']['name']){
                    $pessoa->uploadProfilePhoto($_FILES['profile_photo'], __DIR__ . '/../UPLOADS/profilePhotos', $_POST['last_photo']);
                }else{
                    $pessoa->setProfilePhoto($_POST['last_photo']);
                }
            }
    ...
    case '/POST/usuario/personal-info-update':
        
        try{
            
            $id_usuario = Functions::verifyPost('id_usuario');
            $cpf = Functions::verifyPost('cpf');
            $email = Functions::verifyPost('email');
            $data_aniversario = Functions::verifyPost('date_birth');
            $telefone = $_POST['telefone'] ?? '';
            $senha = $_POST['senha'] ?? false;
            
            $cnpj = $_POST['cnpj'] ?? '';
            $_POST['nivel_acesso'] == 'vendedor' && Functions::verifyPost('cnpj');
            
            $pessoa = new Pessoa();
            $pessoa->setId($id_usuario);
            $pessoa->setCpf($cpf, $id_usuario);
            $pessoa->setEmail($email, $id_usuario);
            $pessoa->setDateBirth($data_aniversario);
            $pessoa->setTelefone($telefone, $id_usuario);
            
            if($cnpj){
                PessoaController::updateCnpj($cnpj, $id_usuario);
            }
            if($senha){
                PessoaController::updatePassword($senha, $id_usuario);
            };
            
            echo json_encode(PessoaController::personalInfoUpdate($pessoa));
        }catch(Exception $e){
            echo $e->getMessage();
        }
    ...
    case '/POST/update/vendedor':
        try{
            ...
            foreach($postArray as $post){
                if($post === 'complemento' || $post === 'weekend') continue;
            ...
            foreach($postArray as $i => $post){
                $setter = $setsArray[$i];
                if($setter === 'setStoreName' || $setter === 'setTelefoneContato'){
    ...
    case '/POST/cart/insert':
        $post_array = ['user_id', 'cor', 'tamanho', 'product_id', 'qty'];
    ...
    case '/GET/compra-by-id':
        ...
        try{
            $id_compra =  Functions::verifyPost('id_compra');

            echo json_encode(CompraController::getById($id_compra), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
        }catch(Exception $e){
            echo $e->getMessage();
        }
    break;

    case '/GET/itens-by-seller_id':
        // $_POST['id_seller'] = 1;

        try{
            Functions::verifyPost('id_seller');
            // $id = json_decode($_POST['id_compra'], true);

            echo json_encode(ItensCompraController::getBySellerId($_POST['id_seller']), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
        }catch(Exception $e){
            echo $e->getMessage();
        }
    break;

    case '/POST/confirmar-item':
        try{

            $id_item = Functions::verifyPost('id_item');
            $quem_entrega = Functions::verifyPost('quem_entrega');
            
            echo json_encode(ItensCompraController::statusConfirm($id_item, $quem_entrega), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    ...
    case '/POST/cancelar_item':
        try{

            $id_item = Functions::verifyPost('id_item');
            $motivo_cancelamento = Functions::verifyPost('motivo_cancelamento');
            $quem_cancelou = Functions::verifyPost('quem_cancelou');
            
            echo json_encode(ItensCompraController::cancelarItem($id_item, $motivo_cancelamento, $quem_cancelou), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    ...
    case '/POST/alterar_status_item':
        try{

            $id_item = Functions::verifyPost('id_item');
            $novo_estado = Functions::verifyPost('novo_estado');

            $statusArray=['em transporte','entregue','chegara hoje','chegou','não recebido'];
